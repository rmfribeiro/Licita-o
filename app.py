...
def _mostrar_logo(largura: int = 130) -> bool:
...
with aba8:
    ...
    if "sanc_parecer" in st.session_state:
        ...
        if _tipo_sanc == "multa":
            ...
            _vb_sanc = _dos_sanc.get("valor_base_calculo")
            if isinstance(_vb_sanc, (int, float)) and _vb_sanc > 0:
                _linhas_dos_sanc.append(
                    ["Valor da base", f"R$ {_vb_sanc:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")]
                )
...
with aba11:
    ...
    if st.button("Analisar e Gerar Minuta de Diligência", type="primary", key="btn_fid_analisar"):
        if not _api_key_fid:
            ...
            except (RuntimeError, ValueError) as _e_fid:
